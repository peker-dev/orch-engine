"""CodexCliAdapter timeout retry smoke.
...
assert 기반 평면 스크립트. claude_cli_retry_smoke 와 같은 형식.
"""
...
_FAKE_STDOUT = json.dumps(_FAKE_VERIFIER_RESULT)


def _make_adapter() -> codex_cli.CodexCliAdapter:
...
def _success_completed() -> subprocess.CompletedProcess:
    return subprocess.CompletedProcess(
        args=["codex"], returncode=0, stdout=_FAKE_STDOUT, stderr=""
    )
...
def _run_with_sequence(sequence: list) -> tuple[list, BaseException | dict | None]:
    ...
    out: dict | None = None
    with mock.patch.object(codex_cli.subprocess, "run", side_effect=fake_run):
        try:
